...
public class RegistryController {
	 ...
	 */
	private void pushPublishServiceToRegistry(RemotingTransporter request, String eachAddress)
		...
		RemotingTransporter remotingTransporter = defaultProvider.getNettyRemotingClient().invokeSync(eachAddress,
				request, 3000);
		if (null != remotingTransporter) {
			AckCustomBody ackCustomBody = SerializerHolder.serializerImpl().readObject(remotingTransporter.bytes(),
					AckCustomBody.class);
